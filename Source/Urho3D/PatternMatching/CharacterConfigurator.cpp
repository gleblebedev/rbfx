...
void CharacterConfigurator::ResetMasterModel()
{
    ...
    if (!characterNode_)
    {
        characterNode_ = node_->CreateChild("CharacterRoot", 0, true);
    }
...
void CharacterConfigurator::ResetBodyStructure()
{
    ...
    animationController_ = characterNode_->GetOrCreateComponent<AnimationController>();
}
...
void CharacterConfigurator::OnNodeSet(Node* prevnode, Node* newnode)
{
    Component::OnNodeSet(prevnode, newnode);
    if (newnode)
    {
        ResetBodyStructure();
    }
